      ...
      fs.readFile(podFile, 'utf8', function (err,data) {
        ...
        if (err) {
          ...
          if (!data.includes("LPMessagingSDK")){
              data = data.replace(/end/g, 'pod \'LPMessagingSDK\', :git => \'https://github.com/medipudipoojitha/iOSPodSpecs.git\'\nend');
          } else {
            data = data.replace(/^.*LPMessagingSDK.*$/mg, 'pod \'LPMessagingSDK\', :git => \'https://github.com/medipudipoojitha/iOSPodSpecs.git\'\n');
          }
